#ifndef __R1CS_LATTICE_SNARK_LPCP_TRANS__
#define __R1CS_LATTICE_SNARK_LPCP_TRANS__
...
namespace libsnark {
    ...
    template <typename pdpT, typename ppT, typename cpT, class Params>
    class r1cs_lattice_snark_crs {
    public:
        r1cs_constraint_system<libff::Fr<pdpT>> constraint_system;
        ...
        explicit r1cs_lattice_snark_crs(
            const r1cs_constraint_system<libff::Fr<pdpT>> &cs,
            std::vector<LWE::Vector<Rq_T<cpT>, Params::pt_dim + Params::tau>>
                &&enc_q,
    ...
    public:
        ...
        std::vector<libff::Fr_vector<pdpT>> A_prefix, B_prefix, C_prefix;
        libff::Fr_vector<pdpT> Z_s;
        ...
        r1cs_lattice_snark_verification_key(
            ...
            std::vector<libff::Fr_vector<pdpT>> &&A_prefix_,
            std::vector<libff::Fr_vector<pdpT>> &&B_prefix_,
            std::vector<libff::Fr_vector<pdpT>> &&C_prefix_,
            libff::Fr_vector<pdpT> &&Z_s_)
            : sk(std::move(sk_)), A_prefix(std::move(A_prefix_)),
              B_prefix(std::move(B_prefix_)), C_prefix(std::move(C_prefix_)),
              Z_s(std::move(Z_s_)) {}
    };

    template <typename pdpT, typename ppT, class Params>
    inline void
    gen_q_mat(const r1cs_constraint_system<libff::Fr<pdpT>> &cs,
              r1cs_lattice_snark_query_matrix<ppT, Params::pt_dim> &_q_mat,
              std::vector<libff::Fr_vector<pdpT>> &A_qs,
              std::vector<libff::Fr_vector<pdpT>> &B_qs,
              std::vector<libff::Fr_vector<pdpT>> &C_qs,
              std::vector<libff::Fr_vector<pdpT>> &A_prefix,
              std::vector<libff::Fr_vector<pdpT>> &B_prefix,
              std::vector<libff::Fr_vector<pdpT>> &C_prefix,
              std::vector<libff::Fr_vector<pdpT>> &H_qs,
              libff::Fr_vector<pdpT> &Z_s) {
        ...
        for (size_t i = 0; i < Params::query_num; i++) {
            qap_instance_evaluation<libff::Fr<pdpT>> qap_inst =
        ...
        r1cs_lattice_snark_query_matrix<pdpT, Params::pt_dim / 2> q_mat;
        ...
        expand_queries<pdpT, ppT, Params::pt_dim / 2>(q_mat, _q_mat);
    ...
    void r1cs_lattice_snark_generator(
        const r1cs_constraint_system<libff::Fr<pdpT>> &cs,
        r1cs_lattice_snark_crs<pdpT, ppT, cpT, Params> &crs,
        r1cs_lattice_snark_verification_key<pdpT, ppT, cpT, Params> &vk) {
        ...
        std::vector<libff::Fr_vector<pdpT>> A_qs, B_qs, C_qs, H_qs;
        libff::Fr_vector<pdpT> Zs;
        std::vector<libff::Fr_vector<pdpT>> A_prefix, B_prefix, C_prefix;
        r1cs_lattice_snark_query_matrix<ppT, Params::pt_dim> _q_mat;
        gen_q_mat<pdpT, ppT, Params>(cs, _q_mat, A_qs, B_qs, C_qs, A_prefix,
                                     B_prefix, C_prefix, H_qs, Zs);
        ...
        crs = r1cs_lattice_snark_crs<pdpT, ppT, cpT, Params>(
            cs, std::move(dummy), std::move(sk_pp.second), _crs_aes_key);
        vk = r1cs_lattice_snark_verification_key<pdpT, ppT, cpT, Params>(
            std::move(sk_pp.first), std::move(A_prefix), std::move(B_prefix),
            std::move(C_prefix), std::move(Zs));
        encrypt_query_matrix<ppT, cpT, Params>(vk.sk, _q_mat, crs.crs_aes_key,
                                               crs.enc_qs);

        libff::leave_block("Generating CRS and VK");
    }

    template <typename pdpT, typename ppT>
    inline void prepare_pi_proof(const qap_witness<libff::Fr<pdpT>> &qap_wit,
                                 libff::Fr_vector<ppT> &pi_v) {
        ...
        pi_v.resize(proof_dim * 2);
        libff::Fr_vector<pdpT> pi(proof_dim);
    ...
    r1cs_lattice_snark_proof<ppT, cpT, Params> r1cs_lattice_snark_prove(
        const r1cs_lattice_snark_crs<pdpT, ppT, cpT, Params> &crs,
        const r1cs_primary_input<libff::Fr<pdpT>> &primary_input,
        const r1cs_auxiliary_input<libff::Fr<pdpT>> &auxiliary_input) {
        ...
        const libff::Fr<pdpT> d1 = libff::Fr<pdpT>::random_element(),
                              d2 = libff::Fr<pdpT>::random_element(),
                              d3 = libff::Fr<pdpT>::random_element();
        public_params_init<ppT, cpT>(original_prg, original_dg);
        delete temp_dg;
        delete temp_prg;

        const qap_witness<libff::Fr<pdpT>> qap_wit = r1cs_to_qap_witness_map(
            crs.constraint_system, primary_input, auxiliary_input, d1, d2, d3);
        libff::leave_block("Compute H polynomial");

        libff::Fr_vector<ppT> pi_v;
        prepare_pi_proof<pdpT, ppT>(qap_wit, pi_v);
        ...
        assert(pi_v.size() == crs.enc_qs->size());
        ...
        for (int i = 0; i < index; i++)
            added.c_vec += crs.enc_qs[i] * pi_v[i];
    ...
    bool r1cs_lattice_snark_verify(
        const r1cs_lattice_snark_verification_key<pdpT, ppT, cpT, Params> &vk,
        const r1cs_primary_input<libff::Fr<pdpT>> &primary_input,
        ...
        libff::Fr_vector<ppT> _decrypted_vec(std::begin(decrypted.vec),
                                             std::end(decrypted.vec));
        libff::Fr_vector<pdpT> _decrypted;
        fp_shrink(_decrypted_vec, _decrypted);
        libff::Fr_vector<pdpT> Ap(Params::query_num), Bp(Params::query_num),
            Cp(Params::query_num), Hp(Params::query_num);

        for (uint i = 0; i < Params::query_num; i++) {
            Ap[i] = _decrypted[i * LWE::query_size];
            Bp[i] = _decrypted[i * LWE::query_size + 1];
            Cp[i] = _decrypted[i * LWE::query_size + 2];
            Hp[i] = _decrypted[i * LWE::query_size + 3];
    ...
    template <typename pdpT, typename ppT, typename cpT, class Params>
    bool run_r1cs_lattice_snark(const r1cs_example<libff::Fr<pdpT>> &example) {
        libff::enter_block("Call to R1CS lattice SNARK");

        libff::print_header("R1CS lattice SNARK Generator");
        r1cs_lattice_snark_crs<pdpT, ppT, cpT, Params> crs;
        r1cs_lattice_snark_verification_key<pdpT, ppT, cpT, Params> vk;
        r1cs_lattice_snark_generator<pdpT, ppT, cpT, Params>(
        ...
        auto proof = r1cs_lattice_snark_prove<pdpT, ppT, cpT, Params>(
        ...
        const bool ans = r1cs_lattice_snark_verify<pdpT, ppT, cpT>(
    ...
    void test_r1cs_lattice_snark(size_t num_constraints, size_t input_size) {
        ...
        r1cs_example<libff::Fr<pdpT>> example =
            generate_r1cs_example_with_field_input<libff::Fr<pdpT>>(
                num_constraints, input_size);
        const bool res =
            run_r1cs_lattice_snark<pdpT, ppT, cpT, Params>(example);
        if (!res)
